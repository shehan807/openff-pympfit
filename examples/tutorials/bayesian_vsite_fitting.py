...
coords, multipoles = Psi4GDMAGenerator.generate(molecule, conformer, settings, minimize=True)
...
targets = [torch.from_numpy(r.astype(np.float64).flatten()) for r in objective_term.reference_values]
...
def model():
    ...
    sigma = pyro.sample("sigma", dist.HalfCauchy(torch.tensor([[0.1]], dtype=torch.float64)))
...
for i, atom in enumerate(molecule.atoms):
    print(f"  {atom.symbol}{i+1}: {atom_q[:, i, 0].mean():+.4f} +/- {atom_q[:, i, 0].std():.4f}")
