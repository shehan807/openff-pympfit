...
DISTINCT_COLORS = [
    (0.90, 0.10, 0.10), (0.10, 0.50, 0.90), (0.15, 0.75, 0.15),
    (0.95, 0.55, 0.00), (0.58, 0.15, 0.80), (0.00, 0.75, 0.75),
    (0.90, 0.20, 0.60), (0.55, 0.35, 0.00), (0.40, 0.75, 1.00),
    (0.80, 0.80, 0.00), (1.00, 0.40, 0.40), (0.00, 0.45, 0.35),
    (0.75, 0.50, 1.00), (0.00, 0.60, 0.30), (0.85, 0.65, 0.15),
    (0.35, 0.35, 0.80), (0.95, 0.75, 0.50),
]
...
def _generate_rdkit_imgs(
    ...
    for name, mol, lab in zip(names, molecules, labels):
        for i, atom in enumerate(mol.atoms):
            equiv[lab[i]].append((name, i))
    shared = {k for k, v in equiv.items() if len(set(t[0] for t in v)) > 1}
    ...
    for mol, lab in zip(molecules, labels):
    ...
    img = Draw.MolsToGridImage(
        rdkit_mols, molsPerRow=2, subImgSize=grid_size, legends=names,
        highlightAtomLists=h_atoms_list, highlightAtomColors=h_colors_list,
    ...
    for name, rdmol, h_atoms, h_colors in zip(
        names, rdkit_mols, h_atoms_list, h_colors_list
    ):
        drawer = rdMolDraw2D.MolDraw2DSVG(*svg_size)
        drawer.drawOptions().addAtomIndices = True
        drawer.DrawMolecule(
            rdmol, highlightAtoms=h_atoms,
